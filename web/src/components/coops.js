...
class CoOps extends Component {
  ...
  componentDidMount() {
    console.log('fetching CoOps')
    axios.get('http://localhost:5002/co_ops')
    .then(results => {
      this.setState({data: results.data.co_ops});
    })
  }
  ...
  render() {
    return (
      <ContentDiv>
        <ContentArea>
          <NewButton href="/CoOps/new">+</NewButton>
          <CoopTable key='header'>
            <HeaderField>Name</HeaderField>
            <HeaderField>Active?</HeaderField>
            {/* <HeaderField>Start Date</HeaderField>
            <HeaderField>End Date</HeaderField> */}
            <HeaderField>Location</HeaderField>
            <HeaderField>Initial Balance</HeaderField>
            <HeaderField>Current Balance</HeaderField>
          </CoopTable>
            {this.state.data.map(({name, is_active, start_date, end_date, location, initial_balance, current_balance}) => {
              return (
                <CoopTable key={name}>
                  <CoopField>{name}</CoopField>
                  <CoopField>{is_active ? 'Yes' : 'No'}</CoopField>
                  {/* <CoopField>{start_date}</CoopField>
                  <CoopField>{end_date}</CoopField> */}
                  <CoopField>{location}</CoopField>
                  <CoopField>{initial_balance}</CoopField>
                  <CoopField>{current_balance}</CoopField>
                </CoopTable>
              )
            })}
        </ContentArea>
